...
This guide provides a step-by-step tutorial on setting up Argo CD for GitOps on Kubernetes.
...
Change the `ClusterIP` of the `argocd-server` service to `NodePort` to access Argo CD from your browser.
...
Open the UI in your browser at the Node IP and port:
...
List secrets to find the admin password:
...
- **Username:** `admin`
- **Password:** _(your decoded password)_
...
**Example repo:** [argoproj/argocd-example-apps/guestbook](https://github.com/argoproj/argocd-example-apps/tree/master/guestbook)

- *Tip:* HEAD is set so that it automatically triggers on the latest commit.
...
**Expanded view:**  
See the full project status, from service files to pod deployment.
...
##  Access the Guestbook App via CLI
